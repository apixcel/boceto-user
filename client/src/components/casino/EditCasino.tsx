import { useUpdateCasinoByIdMutation } from "@/redux/feature/casino/casino.api";
...
interface IProps extends IChildProps {
  selectedCasino: ICasaino | null;
}

const EditCasinot: React.FC<IProps> = ({ setViewMode, selectedCasino }) => {
  const [updateCasino, { isLoading }] = useUpdateCasinoByIdMutation();
  ...
  const handleSubmit = async (values: { name: string }) => {
    ...
    try {
      const res = await updateCasino({
        id: selectedCasino._id || "",
        payload: {
          name: values.name,
        },
      });
